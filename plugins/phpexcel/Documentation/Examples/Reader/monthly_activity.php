...
$inputFileName = './sampleData/Monthly_Activity_Summary_7.15.xls';
	...
	*/
	...
	//{	
		...
		for($j=0;$j<100;$j++)
		{
			...
			if($sheetData[$k][$char] != '' && $sheetData[$k+1][$char] != '')
			{
				$col_cnt=count(array_values($sheetData[$k]));
				$analyst_name=$sheetData[$k][$char];
				$ticker=$sheetData[$k+1][$char];
				...
				for($i=5;$i<=count($sheetData);$i++)
				{
					...
					$votes=$sheetData[$i][$char];
					$acc_name=$sheetData[$i]['B'];
					$last_vote=$sheetData[$i][getNameFromNumber($col_cnt-10)];
					$sales_person=$sheetData[$i][getNameFromNumber($col_cnt-9)];
					$research_rank=$sheetData[$i][getNameFromNumber($col_cnt-8)];
					$overall_rank=$sheetData[$i][getNameFromNumber($col_cnt-7)];
					$vote_type=$sheetData[$i][getNameFromNumber($col_cnt-6)];
					echo 'ANALYST>>'.$analyst_name.'TICKER>> '.$ticker.'VOTES>> '.$votes.'ACC_NAME>> '.$acc_name.'LAST_VOTE>> '.$last_vote.'SALES_MAN>> '.$sales_person.'RR>> '.$research_rank.'OR>> '.$overall_rank.'VOTE>> '.$vote_type.'<br>';
					//echo $sql = "insert into D_MONTHLY_ACTIVITY1 (analyst_name, ticker, account_name, votes, last_vote, salesperson_name, research_rank, overall_rank, vote_type) values('$analyst_name','$ticker','$acc_name','$votes','$last_vote','$sales_person','$research_rank','$overall_rank','$vote_type')";
